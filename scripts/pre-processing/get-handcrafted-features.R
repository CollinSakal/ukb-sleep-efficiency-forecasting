# Script for getting sleep features for everyone in the data
...
get_handcrafted_features <- foreach(eid = eids) %dopar% {
  ...
  source('scripts/utils/get-wearable-features.R')
  ...
  # Save the files if there were no errors
  if("try-error" %in% class(out_dfs)){}else{
    dir_4hrs <- 'data/derived/wearable-features-4hrs/'
    dir_8hrs <- 'data/derived/wearable-features-8hrs/'
    ext_4hrs <- '-wearable-features-4hrs.csv'
    ext_8hrs <- '-wearable-features-8hrs.csv'
    
    fpath_4hrs <- paste0(dir_4hrs,eid,ext_4hrs)
    fpath_8hrs <- paste0(dir_8hrs,eid,ext_8hrs)
    
    write_csv(out_dfs$df_features_4hrs, fpath_4hrs)
    write_csv(out_dfs$df_features_8hrs, fpath_8hrs)
  }
}
